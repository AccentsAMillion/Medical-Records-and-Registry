    ...
    <body>
      ...
      <div id="wrapper">
        <div id="main_content">
            ...
            <hr>
             <div id="wrapper2">
             <legend>ICD-10 Information</legend><br>
             <br><br>
        <form action="search_db_advice.php" method="post"><br>
        ...
        if(isset($_POST['submit'])){
            $name=$_POST['search'];
            $sql="SELECT * FROM advice WHERE  Code LIKE '%" . $name . "%'";// 
            $mydb=mysqli_select_db($conn, $sql);
            $result=mysqli_query($conn, $sql);
        if(mysqli_num_rows($result) > 0) {
            echo mysqli_num_rows($result);
        if ($result->num_rows > 0) {
            echo "<table><tr><th>Code</th><th>Name</th><th>Advice</th></tr>";
            // output data of each row
            while($row = $result->fetch_assoc()) {
                echo "<tr><td>".$row["Code"]."</td><td>".$row["Name"]."</td><td>".$row["Advice"]."</td></tr>";
            }
            echo "</table>";
        } 
        }
