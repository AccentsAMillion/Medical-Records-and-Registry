   ...
   <script src="script.js"></script>
    ...
    <body>
      ...
      <div id="wrapper">
        ...
        /*$sql = "SELECT patient.PatientID, appointment.PhysicianName, appointment.Date, appointment.Time, appointment.Reason, appointment.Cost
        FROM patient, appointment
        INNER JOIN patient
        ON appointment.AppointmentID=patient.PatientID";
        $result = mysqli_query($conn, $sql);*/
         //appointment.PhysicianName, appointment.Date, appointment.Time, appointment.Reason, appointment.Cost
         //INNER JOIN patient
         //ON appointment.PatientID=patient.PatientID
            //
            
        if ($result = mysqli_query($conn, "SELECT * FROM appointment
            ORDER BY PhysicianName;")) {
            $row_cnt = mysqli_num_rows($result);
        if ($row_cnt > 0) {
            echo "<table><tr><th>Patient Name</th><th>Patient ID</th><th>Physician</th><th>Date</th><th>Time</th><th>Reason</th><th>Cost</th></tr>";
        while($row = $result->fetch_assoc()) {
            echo "<tr><td>".$row["PatientName"]."</td><td>".$row["PatientID"]."</td><td>".$row["PhysicianName"]."</td><td>".$row["Date"]."</td><td>".$row["Time"]."</td><td>".$row["Reason"]."</td><td>".$row["Cost"]."</td></tr>";
        }
            echo "</table>";
        ...
        /*if ($result->num_rows > 0) {
            echo "<table><tr><th>Patient ID</th><th>Physician</th><th>Date</th><th>Time</th><th>Reason</th><th>Cost</th></tr>";
            while($row = $result->fetch_assoc()) {
                echo "<tr><td>".$row["PatientID"]."</td><td>".$row["PhysicianName"]."</td><td>".$row["Date"]."</td><td>".$row["Time"]."</td><td>".$row["Reason"]."</td><td>".$row["Cost"]."</td></tr>";
            }
            echo "</table>";
