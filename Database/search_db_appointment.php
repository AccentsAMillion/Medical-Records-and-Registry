      ...
      <div id="wrapper">
        <div id="main_content">
            ...
            <hr>
             ...
             <br><br>
        <form action="search_db_appointment.php" method="post">
        ...
        if(isset($_POST['submit'])){
            $name=$_POST['search'];
            $sql="SELECT * FROM appointment WHERE  PhysicianName LIKE '%" . $name . "%' OR PatientID LIKE '%" . $name  ."%'";// PatientID, PatientName, PhysicianName, Date, Time, Reason, Cost
        ...
        if(mysqli_num_rows($result) > 0) {
         ...
         if ($result->num_rows > 0) {
            echo "<table><tr><th>Patient ID</th><th>Patient Name</th><th>Physician</th><th>Date</th><th>Time</th><th>Reason</th><th>Cost</th></tr>";
            while($row = $result->fetch_assoc()) {
                echo "<tr><td>".$row["PatientID"]."</td><td>".$row["PatientName"]."</td><td>".$row["PhysicianName"]."</td><td>".$row["Date"]."</td><td>".$row["Time"]."</td><td>".$row["Reason"]."</td><td>".$row["Cost"]."</td></tr>";
            }
            echo "</table>";    
        } 
        }
